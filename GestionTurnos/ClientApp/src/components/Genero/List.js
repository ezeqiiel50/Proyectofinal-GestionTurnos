...
const ListGenero = () =>{
    ...
    const [generos,isloging,error] = useGet({url:"api/genero/list?search=",run:run})

    const [data ,setData] = useState([...generos].splice(0,Items_Per_Page));

    const handlerBuscar = () => {
        setSearch(inputSearch.current.value)
    }
    const results = !search ? data : generos.filter((dato) =>
    ...
    const pagHandler = (numberPage) => {
        ...
        if (numberPage >= currentPage) {
            const totalData = generos.length;
            const firstIndex = (numberPage - 1) * Items_Per_Page;
            if (firstIndex === totalData) return;
            setData([...generos].splice(firstIndex, Items_Per_Page))
    ...
    useEffect(() => {
        setData([...generos].splice(0, Items_Per_Page))
        const count_Page = generos.length / Items_Per_Page;
